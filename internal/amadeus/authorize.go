...
func authorize(baseURL string, config utils.Config) (token string, lifespan time.Duration, err error) {

	url := baseURL + "/security/oauth2/token"
	method := "POST"

	id := config.AmadeusClientID
	secret := config.AmadeusClientSecret
